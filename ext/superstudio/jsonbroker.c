...
void Init_jsonbroker()
{
  ...
  rb_define_method(cJsonBroker, "set_depths", json_broker_set_depths, 1);
...
static VALUE json_broker_set_depths(VALUE self, VALUE depths)
{
  ...
  set_depth_array(builder, depths_array, max_depth);
  return Qnil;
}
...
static VALUE json_broker_consume_row(VALUE self, VALUE row)
{
  ...
  consume_row(builder, row_strings, string_sizes);
  return Qnil;
}
