  ...
  )
{
  ...
  level_definitions = found_node->related_parent_level;
  ...
  while(counter < level_definitions->column_count) {
    if (read_type(visible_depth, level_definitions->mapping_array[counter], level_definitions->mapping_array_lengths[counter]) == '1') {
      ...
      // A bunch of this is going to be completely redundant if we push creating these parts of the objects to json_object_array.c
